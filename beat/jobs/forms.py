...
class JobGenForm(forms.Form):
	...
	def change_defaults(self, jfilter):
		self.name		= forms.CharField(max_length=255, required=False, default=jfilter.name)
		self.tool		= forms.ModelChoiceField(Tool.objects.all(), empty_label=None, required=True, default=jfilter.tool)
		self.algoritm	= forms.ModelChoiceField(Algorithm.objects.all(), empty_label=None, required=True, default=jfilter.algorithm)
		self.models		= forms.ModelChoiceField(Model.objects.all().extra(order_by = ['name']), required=True, default=jfilter.model)
		self.options	= forms.CharField(max_length=255, required=False, default=jfilter.options)
